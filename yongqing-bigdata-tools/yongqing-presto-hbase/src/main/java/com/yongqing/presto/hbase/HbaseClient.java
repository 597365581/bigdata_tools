...
import com.yongqing.presto.hbase.conf.HbaseSessionProperties;
import com.yongqing.presto.hbase.conf.HbaseTableProperties;
import com.yongqing.presto.hbase.io.HbasePageSink;
import com.yongqing.presto.hbase.metadata.HbaseTable;
import com.yongqing.presto.hbase.metadata.HbaseView;
import com.yongqing.presto.hbase.metadata.ZooKeeperMetadataManager;
...
import com.yongqing.presto.hbase.serializers.HbaseRowSerializerUtil;
...
public final class HbaseClient {
            ...
            List<HbaseColumnConstraint> constraints) //HbaseRowSerializer serializer
    {
        try {
            ...
            LOG.info("Number of splits for table %s is %d with %d ranges", tableName, tabletSplits.size(), rowIdRanges.size());
            return tabletSplits;
        } catch (Exception e) {
            LOG.error(e);
            throw new PrestoException(HbaseErrorCode.UNEXPECTED_HBASE_ERROR, "Failed to get splits from Hbase", e);
        }
    }
    ...
    public RecordReader<ImmutableBytesWritable, Result> execSplit(ConnectorSession session, HbaseSplit split, List<HbaseColumnHandle> columnHandles)
        ...
        LOG.debug("execSplit : %s", tableName.toString());
    ...
    private static Scan getScanFromPrestoRange(Range prestoRange)
        ...
        if (prestoRange.isAll()) { //全表扫描  all rowkey
            ...
            hbaseScan.setStartRow(HbaseRowSerializerUtil.toHbaseBytes(type, value));
            hbaseScan.setStopRow(HbaseRowSerializerUtil.toHbaseBytes(type, value));
        } else {
            if (prestoRange.getLow().isLowerUnbounded()) {
                ...
                //------- set start and stop -----
                hbaseScan.setStartRow(HbaseRowSerializerUtil.toHbaseBytes(type, startSplit));
                hbaseScan.setStopRow(HbaseRowSerializerUtil.toHbaseBytes(type, endSplit));
            }
        }
    ...
    public HbaseTable getTable(SchemaTableName table) {
        LOG.info("getTable SchemaTableName : " + table.toString());
    ...
    public HbaseTable createTable(ConnectorTableMetadata meta) {
        LOG.info("ConnectorTableMetadata :" + meta.toString());
    ...
    public void dropTable(HbaseTable table) {
        LOG.info("dropTable : %s ", table.toString());

        String metaTableName = table.getTable().contains(".") ? table.getTable().replaceFirst("\\.", "__") : table.getTable();
        LOG.info("dropTable metaTableName : %s ", metaTableName);
        SchemaTableName metaTable = new SchemaTableName(table.getSchema(), metaTableName);

        // Remove the table metadata from Presto
        if (metaManager.getTable(metaTable) != null) {
            LOG.info("dropTable deleteTableMetadata: %s ", metaTable.toString());
            metaManager.deleteTableMetadata(metaTable);
        }
        ...
        if (!table.isExternal()) {
            ...
            String hbaseTableName = table.getSchema() + ":" + table.getTable();
            if (tableManager.exists(hbaseTableName)) {
                LOG.info("dropTable deleteHbaseTable: %s ", hbaseTableName);
                tableManager.deleteHbaseTable(hbaseTableName);
            }
        }
    }
    ...
    private void validateInternalTable(ConnectorTableMetadata meta) {
        ...
        if (tableManager.exists(table)) {
            throw new PrestoException(HbaseErrorCode.HBASE_TABLE_EXISTS, "Cannot create internal table when an Hbase table already exists");
        }
    }
