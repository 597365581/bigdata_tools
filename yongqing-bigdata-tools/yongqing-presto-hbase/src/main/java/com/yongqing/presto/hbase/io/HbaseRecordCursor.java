...
import com.yongqing.presto.hbase.Types;
...
import static com.yongqing.presto.hbase.HbaseErrorCode.IO_ERROR;
import static com.yongqing.presto.hbase.serializers.HbaseRowSerializerUtil.getBlockFromArray;
import static com.yongqing.presto.hbase.serializers.HbaseRowSerializerUtil.getBlockFromMap;
...
public class HbaseRecordCursor
    ...
    public long getLong(int field) {
        ...
        LOG.info("getLong field %s", field);
        byte[] bytes = getValue(field);

        try {
            return Long.parseLong(new String(bytes, "UTF-8"));
        } catch (UnsupportedEncodingException e) {
            LOG.error("getLong parseLong error %s", field);
        }
