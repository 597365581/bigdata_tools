...
public class EtcdUtil {
    ...
    public static String getEtcdValueByKey(String key) throws Exception {
        List<KeyValue> kvs = EtcdUtil.getEtclClient().getKVClient().get(ByteSequence.from(key, Charsets.UTF_8)).get().getKvs();
        if (kvs.size() > 0) {
            String value = kvs.get(0).getValue().toString(Charsets.UTF_8);
            return value;
        } else {
            return null;
        }
    ...
    public static long putEtcdKeyWithExpireTime(String key, String value, long expireTime) throws Exception {
        ...
        EtcdUtil.getEtclClient().getKVClient().put(ByteSequence.from(key, Charsets.UTF_8), ByteSequence.from(value, Charsets.UTF_8), putOption);
    ...
    public static long putEtcdKeyWithLeaseId(String key, String value, long leaseId) throws Exception {
        ...
        CompletableFuture<PutResponse> putResponse = EtcdUtil.getEtclClient().getKVClient().put(ByteSequence.from(key, Charsets.UTF_8), ByteSequence.from(value, Charsets.UTF_8), putOption);
        return putResponse.get().getHeader().getRevision();
    }

    public static void keepAliveEtcdSingleLease(long leaseId, StreamObserver<LeaseKeepAliveResponse> streamObserver) {
        EtcdUtil.getEtclClient().getLeaseClient().keepAlive(leaseId, streamObserver);
    }

    public static Watch.Watcher getCustomWatcherForSingleKey(String key, Watch.Listener listener) {
        return EtcdUtil.getEtclClient().getWatchClient().watch(ByteSequence.from(key, Charsets.UTF_8), listener);
    }

    public static Watch.Watcher getCustomWatcherForPrefix(String prefix, Watch.Listener listener) {
        WatchOption watchOption = WatchOption.newBuilder().withPrefix(ByteSequence.from(prefix, Charsets.UTF_8)).build();
        return EtcdUtil.getEtclClient().getWatchClient().watch(ByteSequence.from(prefix, Charsets.UTF_8), watchOption, listener);
    }

    public static void deleteEtcdKeyWithPrefix(String prefix) {
        DeleteOption deleteOption = DeleteOption.newBuilder().withPrefix(ByteSequence.from(prefix, Charsets.UTF_8)).build();
        EtcdUtil.getEtclClient().getKVClient().delete(ByteSequence.from(prefix, Charsets.UTF_8), deleteOption);
    }

    public static List<KeyValue> getEtcdKeyWithPrefix(String prefix) {
        List<KeyValue> keyValues = new ArrayList<>();
        GetOption getOption = GetOption.newBuilder().withPrefix(ByteSequence.from(prefix, Charsets.UTF_8)).build();
        try {
            keyValues = EtcdUtil.getEtclClient().getKVClient().get(ByteSequence.from(prefix, Charsets.UTF_8), getOption).get().getKvs();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return keyValues;
    ...
    public static void putEtcdValueByKey(String key, String value) throws Exception {
        EtcdUtil.getEtclClient().getKVClient().put(ByteSequence.from(key, Charsets.UTF_8), ByteSequence.from(value.getBytes("utf-8")));
    }
    ...
    public static void deleteEtcdValueByKey(String key) {
        EtcdUtil.getEtclClient().getKVClient().delete(ByteSequence.from(key, Charsets.UTF_8));
    }
    ...
    public static void initListen(String key) {
        try {
            ...
            //加载配置
            loadProperties(getConfig(EtcdUtil.getEtclClient().getKVClient().get(ByteSequence.from(key, Charsets.UTF_8)).get().getKvs()));
            Watch.Watcher watcher = EtcdUtil.getEtclClient().getWatchClient().watch(ByteSequence.from(key, Charsets.UTF_8), new Watch.Listener() {
                @Override
                public void onNext(WatchResponse watchResponse) {
                    watchResponse.getEvents().stream().forEach(watchEvent -> {
                        KeyValue kv = watchEvent.getKeyValue();
                        log.info("etcd event:{} ,change key is:{},afterChangeValue:{}", watchEvent.getEventType(), kv.getKey().toString(Charsets.UTF_8), kv.getValue().toString(Charsets.UTF_8));
                        loadProperties(kv.getValue().toString(Charsets.UTF_8));
                    });
                }

                @Override
                public void onError(Throwable throwable) {
                    log.error("etcd listen start cause Exception:{}", throwable);
                }

                @Override
                public void onCompleted() {
                    log.info("etcd listen onCompleted success...");
                }
            });
        } catch (Exception e) {
    ...
    private static String getConfig(List<KeyValue> kvs) {
        if (kvs.size() > 0) {
            String config = kvs.get(0).getKey().toString(Charsets.UTF_8);
            String value = kvs.get(0).getValue().toString(Charsets.UTF_8);
    ...
    public static <T, R> R distributedLockExec(Exec exec, T parameters, String name, long ttl, Long timeOutMs) {
        ...
        try {
            ...
            if (null != timeOutMs && timeOutMs > 0) {
                feature = lock.lock(ByteSequence.from(name, Charsets.UTF_8), EtcdUtil.getEtclClient().getLeaseClient().grant(ttl).get(timeOutMs, TimeUnit.MILLISECONDS).getID());
            } else {
                feature = lock.lock(ByteSequence.from(name, Charsets.UTF_8), EtcdUtil.getEtclClient().getLeaseClient().grant(ttl).get().getID());
            }
